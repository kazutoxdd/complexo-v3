...
function checkVehicleEngine()
	CreateThread(function()
		while true do
			Wait(500)
			local playerPed = PlayerPedId()
			if not IsPedInAnyVehicle(playerPed, false) and inVehicle then
				SetVehicleEngineOn(vehicle, false, true, false)
				Wait(1000)
				local Plate = GetVehicleNumberPlateText(vehicle)
				if fuelVeh[Plate] ~= nil then
					TriggerServerEvent("engine:tryFuel", Plate, fuelVeh[Plate])
					Entity(vehicle)["state"]:set("Fuel", fuelVeh[Plate], true)
				end
				inVehicle = false
			end
		end
		...
		if IsPedInAnyVehicle(ped) then
			...
			if GetVehicleFuelLevel(vehicle) >= 1 then
				if speed >= 1 then
					...
					if fuelVeh[Plate] ~= nil and fuelVeh[Plate] ~= 'PDMSPORT' then
					...
					if DoesEntityExist(vehicle) and not vehiclesBlacklist[vehModel] then
						...
						if distance <= 3.5 then
							...
							if not isFuel then
								...
								if GetAmmoInPedWeapon(ped, 883325847) - 0.02 * 100 > 1 then
									SetPedAmmo(ped, 883325847,
										math.floor(GetAmmoInPedWeapon(ped, 883325847) - 0.02 * 100))
									...
									text3D(coordsVeh["x"], coordsVeh["y"], coordsVeh["z"] + 0.85,
										...
										"%   ~w~GALÃO: ~y~" ..
										parseInt(GetAmmoInPedWeapon(ped, 883325847) / 4500 * 100) .. "%")

									if not IsEntityPlayingAnim(ped, "timetable@gardener@filling_can", "gar_ig_5_filling_can", 3) then
										TaskPlayAnim(ped, "timetable@gardener@filling_can", "gar_ig_5_filling_can", 3.0,
											3.0, -1, 50, 0, 0, 0, 0)
									end
							...
							if DoesEntityExist(vehicle) and not vehiclesBlacklist[vehModel] then
								...
								if not pumpGunInHand then
									for _, p in ipairs(gasPumpModels) do
										LastPump = GetClosestObjectOfType(coords.x, coords.y, coords.z, 2.5, p[1], false,
											false, false)
								...
								if not pumpGunInHand and distanceToPump < 2.0 and vehFuel < 100.0 then
									...
									if IsControlJustPressed(1, 38) and GetGameTimer() >= gameTimer then
										...
										AttachEntityToEntity(currentPumpEntity, PlayerPedId(),
											GetPedBoneIndex(PlayerPedId(), 60309), 0.055, 0.05, 0.0, -50.0, -90.0, -50.0,
											true, true, false, true, 0, true)
										local RopePumpOffset = GetOffsetFromEntityInWorldCoords(currentPumpEntity, 0.0,
											-0.02, -0.175)
										local RopeLength = 3.5
										currentPumpRope = AddRope(PumpCoords.x + PumpConfig[2],
											PumpCoords.y + PumpConfig[3], PumpCoords.z + PumpConfig[4], 0.0, 0.0, 0.0,
											RopeLength, 4, RopeLength, 1.0, 0.0, false, false, false, 2.0, 0)
										AttachEntitiesToRope(currentPumpRope, LastPump, currentPumpEntity,
											PumpCoords.x + PumpConfig[2], PumpCoords.y + PumpConfig[3],
											PumpCoords.z + PumpConfig[4], RopePumpOffset.x, RopePumpOffset.y,
											RopePumpOffset.z, RopeLength, false, false, nil, nil)
								...
								if pumpGunInHand and distanceToVeh <= 3.5 then
									if not isFuel then
										...
										if pumpGunInCar and vehFuel < 100.0 then
											text3D(currentPropWheelCoords["x"], currentPropWheelCoords["y"],
												currentPropWheelCoords["z"] + 1, "~g~E~w~   ABASTECER")
										end
									else
										if not showNui then
										...
										if lastGameTimer < GetGameTimer() and GetIsVehicleEngineRunning(vehicle) then
											...
											if Chance < 39 then
												explodePump = true
												AddExplosion(coords.x, coords.y, coords.z, 34, 1.0, 1.0, true, true,
													false)
											end
										end
										...
										if vehFuel >= 100.0 or GetEntityHealth(ped) <= 101 or explodePump then
											...
											StopAnimTask(ped, "timetable@gardener@filling_can", "gar_ig_5_filling_can",
												2.0)
											RemoveAnimDict("timetable@gardener@filling_can")
									...
									if IsControlJustPressed(1, 38) and GetGameTimer() >= gameTimer and pumpGunInHand and pumpGunInCar then
										...
										if isFuel then
											...
											if vehFuel < 100.0 and distanceToPumpWheel < 2.5 then
												...
												TaskTurnPedToFaceEntity(ped, vehicle, 5000)
												loadAnim("timetable@gardener@filling_can")
												TaskPlayAnim(ped, "timetable@gardener@filling_can",
													"gar_ig_5_filling_can", 3.0, 3.0, -1, 50, 0, 0, 0, 0)
												isFuel = true
												lastVehicleInFuel = vehicle
												lastVehicleIsPlane = IsThisModelAPlane(vehModel) or
													IsThisModelAHeli(vehModel)
											end
										end
									end
									...
									if pumpGunInHand and not pumpGunInCar then
										currentPropWheelCoords = GetWorldPositionOfEntityBone(vehicle,
											GetEntityBoneIndexByName(vehicle, "wheel_lr"))
										local distanceToPumpWheel = #(coords - currentPropWheelCoords)
										text3D(currentPropWheelCoords.x, currentPropWheelCoords.y,
											currentPropWheelCoords.z + 0.7, "~g~E~w~   ENCAIXAR")
										...
										if IsControlJustPressed(1, 38) and GetGameTimer() >= gameTimer and distanceToPumpWheel < 2.5 + (lastVehicleIsPlane and 20.0 or 0.0) then
											...
											local positionOffset = GetOffsetFromEntityGivenWorldCoords(vehicle,
												currentPropWheelCoords.x, currentPropWheelCoords.y,
												currentPropWheelCoords.z + 0.7)
											AttachEntityToEntity(currentPumpEntity, vehicle, -1,
												positionOffset.x - (isABike and 0.25 or 0.1),
												positionOffset.y + (isABike and 0.9 or 0.0),
												positionOffset.z + (isABike and 0.1 or 0.0), isABike and -90.0 or -50.0,
												0.0, -90.0, true, true, false, false, 0, true)
								...
								if isFuel and distanceToVeh > 3.5 then
									...
									SetTimeout(3000, function()
										...
										if distanceToPump > 7.0 + (lastVehicleIsPlane and 20.0 or 0.0) and pumpGunInCar then
											explodePump = true
											AddExplosion(lasTcoords.x, lasTcoords.y, lasTcoords.z, 34, 1.0, 1.0, true,
												true, false)
										end
										lastVehicleInFuel = nil
										lastVehicleIsPlane = nil
										...
										DeleteRope(currentPumpRope)
										LastPump = nil
										StopAnimTask(PlayerPedId(), "timetable@gardener@filling_can",
											"gar_ig_5_filling_can", 2.0)
										RemoveAnimDict("timetable@gardener@filling_can")
									end)
								end
...
AddEventHandler("engine:vehTuning", function()
	...
	if Vehicle then
		...
		TriggerEvent("Notify", "azul",
			...
			"%<br><b>Motor:</b> " .. parseInt(engine / 10) .. "%<br><b>Gasolina:</b> " .. parseInt(fuel) .. "%", "Aviso",
			10000)
	end
end)