...
local Blips = {
	...
	{ -1228.4,-1459.22,4.31,78,11,"Market",0.5 },
...
local function blockJump()
    local block = true
    SetTimeout(5000, function() block = false end)
...
RegisterCommand("jump", function()
    if not IsControlEnabled(0, 22) then return end
    if IsPedInAnyVehicle(ply) then return end
    local t = GetGameTimer() + 500
    while not IsPedJumping(ply) and GetGameTimer() < t do Wait(50) end
    if not IsPedJumping(ply) then return end
    blockJump()
end)
-----------------------------------------------------------------------------------------------------------------------------------------
-- REGISTERKEYMAPPING
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterKeyMapping("jump", "Pular", "keyboard", "space")
-----------------------------------------------------------------------------------------------------------------------------------------
-- ALPHAS
-----------------------------------------------------------------------------------------------------------------------------------------
local Alphas = {
