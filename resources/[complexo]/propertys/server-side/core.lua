				...
				if os.time() > Consult[1]["Tax"] then
					if vRP.Request(source,"Hipoteca atrasada, deseja efetuar o pagamento?","Sim, concluir pagamento","Não, pago depois") then
						if vRP.PaymentFull(Passport,Informations[Consult[1]["Interior"]]["Price"] * 0.1) then
							...
							TriggerClientEvent("Notify",source,"amarelo","Pagamento concluído.",5000,"Alerta")
						end
					end
				else
	...
	if Passport then
		...
		if Consult[1] then
			if parseInt(Consult[1]["Passport"]) == Passport or vRP.InventoryFull(Passport,"propertys-"..Consult[1]["Serial"]) then
				if Lock[Name] then
					Lock[Name] = nil
					TriggerClientEvent("Notify",source,"amarelo","Propriedade trancada.",5000,"Alerta")
				else
					Lock[Name] = true
					TriggerClientEvent("Notify",source,"amarelo","Propriedade destrancada.",5000,"Alerta")
				end
			end
		end
	end
		...
		if Consult[1] then
			if parseInt(Consult[1]["Passport"]) == Passport then
				...
				if vRP.Request(source,"Deseja vender a propriedade?","Sim, concluir a venda","Não, mudeia de ideia") then
					...
					TriggerClientEvent("Notify",source,"amarelo","Venda concluída.",5000,"Alerta")
	...
	if Passport then
		...
		if Split[1] == "save" then
			...
			if Keyboard then
				...
				if not Consult[NameCheck] then
					...
					TriggerClientEvent("Notify",source,"verde","<b>"..Name.."</b> adicionado.",5000,"Sucesso")
				else
					TriggerClientEvent("Notify",source,"amarelo","Nome escolhido já existe em seu armário.",5000,"Alerta")
				end
			end
		elseif Split[1] == "delete" then
			if Consult[Name] then
				...
				vRP.SetSrvData("Wardrobe:"..Passport,Consult,true)
				TriggerClientEvent("propertys:ClothesReset",source)
				TriggerClientEvent("Notify",source,"verde","<b>"..Name.."</b> removido.",5000,"Sucesso")
			else
				TriggerClientEvent("Notify",source,"amarelo","A vestimenta salva não se encontra mais em seu armário.",5000,"Alerta")
			end
		elseif Split[1] == "apply" then
			if Consult[Name] then
				TriggerClientEvent("skinshop:Apply",source,Consult[Name])
				TriggerClientEvent("Notify",source,"verde","<b>"..Name.."</b> aplicado.",5000,"Sucesso")
			else
				TriggerClientEvent("Notify",source,"amarelo","A vestimenta salva não se encontra mais em seu armário.",5000,"Alerta")
			end
		end
