...
func = Tunnel.getInterface("SavaFy")
vSERVER = Tunnel.getInterface("SavaFy")
Tunnel.bindInterface("SavaFy", fclient)
fclient = {}
local Music = {}
local datasoundinfo = {}
local nuiaberto = false
local nuiaberto2 = false
local myjob = nil
local nomidaberto
local nomidaberto2
local SoundsPlaying = {}
...
Citizen.CreateThread(function()
	TriggerServerEvent("SavaFy:GetDate")
end)
...
RegisterNUICallback("action", function(data)
	...
	elseif data.action == "play" then
		if xSound:soundExists(nameid) then
			if xSound:isPaused(nameid) then
				TriggerServerEvent("SavaFy:ChangeState", true, nameid)
				local esperar = 0
				while nuiaberto do
					Wait(1000)
	...
	elseif data.action == "pause" then
		if xSound:soundExists(nameid) then
			if xSound:isPlaying(nameid) then
				TriggerServerEvent("SavaFy:ChangeState", false, nameid)
			end
		end
	...
	elseif data.action == "loop" then
		if xSound:soundExists(nameid) then
			...
			TriggerServerEvent("SavaFy:ChangeLoop",nameid,datasoundinfo.loop)
		else
	...
	elseif data.action == "forward" then
		if xSound:soundExists(nameid) then
			TriggerServerEvent("SavaFy:ChangePosition", 10, nameid)
		end
...
function ApplySound(quanti,plate)
	...
	if vadi <= 1.01 and vadi >= -0.001 and exis then
		...
		TriggerServerEvent("SavaFy:ChangeVolume", quanti, plate)
	end
end
...
function SetUrl(url,nid)
	...
	if url then
		...
		if encontrad then
			...
			TriggerServerEvent("SavaFy:ModifyURL",vehdata)
		else
			if IsPedInAnyVehicle(PlayerPedId(), false) then
				local veh = GetVehiclePedIsIn(PlayerPedId(),false)
				...
				vehdata.loop = datasoundinfo.loop
				TriggerServerEvent("SavaFy:AddVehicle",vehdata)
			end
		end
	else
...
RegisterNetEvent("SavaFy:ShowNui")
	AddEventHandler("SavaFy:ShowNui", function()
	show()
end)
RegisterNetEvent("SavaFy:ShowNui2")
	AddEventHandler("SavaFy:ShowNui2", function()
	show2()
end)
...
function show2(nomecenas)
    ...
    if shown2  then
		...
		local prop = "rojo_jblboombox"
			...
			--vRP._CarregarObjeto("anim@heists@narcotics@trash","walk","prop_boombox_01",49,28422)
			vRP.CreateObject("anim@heists@box_carry@","walk","prop_boombox_01",49,28422)
...
AddEventHandler("SavaFy:AddVehicle", function(data)
...
AddEventHandler("SavaFy:ChangeState", function(tipo, nome)
...
AddEventHandler("SavaFy:ChangeLoop", function(tipo, nome)
	if xSound:soundExists(nome) then
...
AddEventHandler("SavaFy:ChangeVolume", function(som, range, nome)
	local carroe
	local crds
    for i = 1, #Zones do
        local v = Zones[i]
...
RegisterNetEvent("SavaFy:SendData")
AddEventHandler("SavaFy:SendData", function(data)
    Zones = data
    for i = 1, #Zones do
		local v = Zones[i]
		if v.isplaying then
			...
			--if myjob.name == v.job then
				...
				if distance <= 10 then
					...
					if distance <= 3 then
						...
						--local ped = vRP.getUserId({player})
						DrawText3D(v.changemusicblip.x, v.changemusicblip.y, v.changemusicblip.z, "~r~E~w~ - Trocar musica")
						--print(ped)
						--print(json.encode(v))
						if IsControlJustReleased(0, 38) and func.checkPermission() then
							nomidaberto = v.name
				...
				--if v.job == nil then
					...
					if distance <= 10 then
						...
						if distance <= 3 then
							...
							if IsControlJustReleased(0, 38) and vRP.HasPermission(Passport, cfg.permissao) then
								nomidaberto = v.name
								show(v.name)
								Wait(1000)
							end
