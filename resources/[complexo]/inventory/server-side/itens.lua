...
Use = {
	["contrabandbox1"] = function(source,Passport,Amount,Slot,Full,Item,Split)
		if (vRP.InventoryWeight(Passport) + itemWeight(Full)) <= vRP.GetWeight(Passport) then
			...
			TriggerClientEvent("Notify",source,"Mochila cheia.","vermelho",5000)
		end
	end,
	...
	["weaponbox"] = function(source,Passport,Amount,Slot,Full,Item,Split)
		if (vRP.InventoryWeight(Passport) + itemWeight(Full)) <= vRP.GetWeight(Passport) then
			if vRP.TakeItem(Passport,Full,1,true,Slot) then
				...
				vRP.GenerateItem(Passport,"riflebody",math.random(2,3))
				TriggerClientEvent("inventory:Update",source,"Backpack")
			end
		else
			TriggerClientEvent("Notify",source,"Mochila cheia.","vermelho",5000)
		...
		repeat
			if os.time() >= parseInt(Active[Passport]) then
				...
				if vRP.TakeItem(Passport,Full,1,true,Slot) then
					...
					TriggerClientEvent("inventory:Update",source,"Backpack")
					TriggerClientEvent("Notify",source,"<b>"..itemName("backpack").."</b> usada.","verde",5000)
				end
			end
		...
		if not Player(source)["state"]["Handcuff"] then
			...
			if Vehicle then
				...
				if string.sub(Plate,1,4) == "DISM" then
					...
					if vT3LOCK.getResultLockpick(source) then
		...
		if not Player(source)["state"]["Handcuff"] then
			...
			if Vehicle and vRPC.InsideVehicle(source) then
				if not exports["garages"]:Signal(Plate) then
					...
					TriggerClientEvent("Notify",source,"Atenção","<b>Bloqueador de Sinal</b> já instalado.","amarelo",5000)
				end
			end
		end
	end,
	...
	["fishingrod"] = function(source,Passport,Amount,Slot,Full,Item,Split)
		if vCLIENT.Fishing(source) then
			...
			TriggerClientEvent("Notify",source,"Atenção","Precisa estar na Área de <b>Pesca</b>.","amarelo",5000)
		end
	end,
	...
	["joint"] = function(source,Passport,Amount,Slot,Full,Item,Split)
		if vRP.ConsultItem(Passport,"lighter",1) then
			...
			TriggerClientEvent("Notify",source,"Atenção","Precisa de <b>1x "..itemName("lighter").."</b>.","amarelo",5000)
		end
	end,
	...
	["premium"] = function(source,Passport,Amount,Slot,Full,Item,Split)
		if not vRP.UserPremium(Passport) then
			if vRP.TakeItem(Passport,Full,1,true,Slot) then
				...
				TriggerClientEvent("Notify",source,"Parabéns","Você ativou <b>Premium</b>.","verde",5000)
			end
		else
			if vRP.HasPermission(Passport,"Premium",1) and vRP.TakeItem(Passport,Full,1,true,Slot) then
				...
				if vRP.GetWeight(Passport) >= 125 then
					TriggerClientEvent("Notify",source,"Sua mochila de <b>25kg</b> não foi adicionada pois você atingiu o limite.","amarelo",5000)
					return
				end
		...
		if vehicle[1] then
			...
			if Keyboard then
				...
				if string.len(plateCheck) ~= 8 then
					TriggerClientEvent("Notify",source,"O nome de definição para a placa inválida.","vermelho",5000)
					return
				else
					if vRP.PassportPlate(namePlate) then
						TriggerClientEvent("Notify",source,"Atenção","A placa escolhida já possui em outro veículo.","amarelo",5000)
						return
					else
						if vRP.TakeItem(Passport,Full,1,true,Slot) then
		...
		if Keyboard then
			...
			if string.len(Fir) == 3 and string.len(Sec) == 3 then
				if not vRP.UserPhone(Keyboard[1].."-"..Keyboard[2]) then
					if vRP.TakeItem(Passport,Full,1,true,Slot) then
						TriggerClientEvent("Notify",source,"Sucesso","Telefone atualizado.","verde",5000)
						TriggerClientEvent("inventory:Update",source,"Backpack")
						vRP.UpgradePhone(Passport,Keyboard[1].."-"..Keyboard[2])
					end
				else
					TriggerClientEvent("Notify",source,"Atenção","O número escolhido já possui um proprietário.","amarelo",5000)
				end
			else
				TriggerClientEvent("Notify",source,"O número telefônico deve conter 6 dígitos e somente números.","vermelho",5000)
			end
		end
	end,
				...
				if OtherPassport then
					if vRP.HasService(OtherPassport,"Police") then
						if vRP.TakeItem(Passport,Full,1,true,Slot) then
							...
							TriggerClientEvent("Notify",source,"Atenção","Todas as comunicações foram retiradas.","amarelo",5000)
						end
					end
				...
				repeat
					if os.time() >= parseInt(Active[Passport]) then
						...
						if vRP.TakeItem(Passport,Full,1,true,Slot) then
							...
							TriggerClientEvent("Notify",source,"Kit Quimico","<b>Químicos:</b> "..chemStr.."<br><b>Álcool:</b> "..alcoholStr.."<br><b>Drogas:</b> "..weedStr,"amarelo",10000)
						end
					end
