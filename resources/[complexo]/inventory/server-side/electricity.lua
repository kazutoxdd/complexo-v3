				...
				if vRP.ConsultItem(Passport, "pliers", 1) then
					if vRP.TakeItem(Passport, "c4", 3, true) then
								...
								if os.time() >= parseInt(Active[Passport]) then
									...
									SetTimeout(10000, function()
										...
										TriggerClientEvent("Notify", -1, "azul",
											"Uma forte explosão em nossa central causou um apagão em toda a cidade, estamos trabalhando para reestabelecer os serviços. O sistema de policia se encontra indisponivel e todos os procurados foram liberados",
											"Central de Eletricidade", 30000)

										for Passports, Sources in pairs(vRP.Players()) do
											TriggerEvent('Remove:Wante', Source, Passports)
										end
									end)
						...
						TriggerClientEvent("Notify", source, "amarelo",
							"Você precisa de <b>3x " .. itemName("c4") .. "</b>.", "Atenção", 5000)
					end
				else
