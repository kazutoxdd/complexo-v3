...
local Chairs = {
	...
	[-853526657] = { ["OffsetX"] = 0.0, ["OffsetY"] = 0.0, ["OffsetZ"] = 0.0, ["Heading"] = 180.0 },
	[1545434534] = { ["OffsetX"] = 0.0, ["OffsetY"] = 0.0, ["OffsetZ"] = 0.5, ["Heading"] = 180.0 },
...
CreateThread(function()
	...
	AddTargetModel(
		{ -1235256368, 538002882, -1118419705, -377849416, -109356459, -1692811878, -729914417, 1577885496, 1889748069,
			1816935351, 1037469683, 536071214, 438342263, 2129125614, -853526657, 1545434534 }, {
			options = {
				{
					event = "target:Chair",
					label = "Sentar",
...
AddEventHandler("target:SitChair", function(Number)
	if not Previous then
		...
		TaskStartScenarioAtPosition(Ped, "PROP_HUMAN_SEAT_CHAIR_UPRIGHT", Coords["x"], Coords["y"], Coords["z"],
			SitChairs[Number]["Heading"] + 1.0, -1, true, true)
...
AddEventHandler("target:Chair", function(Vars)
	...
	if Chairs[Model] then
		...
		TaskStartScenarioAtPosition(Ped, "PROP_HUMAN_SEAT_CHAIR_UPRIGHT", Vars[4]["x"] + Chairs[Model]["OffsetX"],
			Vars[4]["y"] + Chairs[Model]["OffsetY"], Vars[4]["z"] + Chairs[Model]["OffsetZ"],
			GetEntityHeading(Entitys) - Chairs[Model]["Heading"], 0, true, true)
	end
end)
