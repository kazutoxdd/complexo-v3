...
local List = {
	["carjack"] = {
		...
		["Description"] = "Um elevador hidráulico usado para levantar veículos.",
		["Type"] = "Usável",
		["Economy"] = 125,
		["Weight"] = 0.15,
		["Max"] = 1
...
function buildInventoryItem(Item)
    local Split = splitString(Item,"-")
    local Item = Split[1]
    if not List[Item] then return {} end
    local data = {
        amount = parseInt(Item.amount),
        name = List[Item].Name or "Deletado",
        peso = List[Item].Weight or 0.0,
        index = List[Item].Index,
        max = List[Item].Max or nil,
        type = List[Item].Type,
        description = List[Item].Description or nil,
        economy = List[Item].Economy or "S/V",
        key = Item.Item,
    }
    data.Durability = 0
    data.days = 1
    if Split[2] then
        if List[Item].Durability then
            data.Durability = parseInt(os.time() - Split[2])
            data.days = List[Item].Durability or false
            data.serial = Split[3]
        else
            data.serial = Split[2]
        end
